...
from calculate_yields import (core_collapse_supernova_yields,
...
class starParticle:
	...
	def __init__(self, age, mass, mass_scaling_factor, kind,
				 composition):
		self.age = age
		self.mass = mass
...
def sample_mass():
	...
	p = imf_power
	u = uniform(0, 1)
	mass = ((u * (m_u ** (p + 1) - m_l ** (p + 1)) + m_l ** (p + 1)) **
			(1 / (p + 1)))
...
def form_stars(num_stars, gas_mass, galaxy_age, t):
	...
	for i in range(num_stars):
		...
		star = starParticle(star_age, star_mass,
							star_mass_scaling_factor, star_kind,
							star_composition)
		stars.append(star)
...
def sort_stars(stars, mortal_stars, immortal_stars,
			   mass_scaling_factor, t):
	"""
...
def explode_stars(mortal_stars, black_holes, white_dwarfs,
				  neutron_stars):
	"""
...
def core_collapse_supernova(star):
	...
	ejecta = (((star.mass - remnant_mass) * star.mass_scaling_factor) *
...
def planetary_nebula(star):
	...
	star.age = 0.0
	ejecta = (star.mass_scaling_factor * (star.mass - 0.6) *
...
def type_ia_supernova(wd_remnant):
	...
	"""
	ejecta = (wd_remnant.mass * wd_remnant.mass_scaling_factor *
...
def binary_neutron_star_merger(ns_remnant):
	...
	"""
	ejecta = (ns_remnant.mass * ns_remnant.mass_scaling_factor *
			  binary_neutron_star_merger_yields(elements))

	return ejecta


def evolve_stars(mortal_stars, black_holes, white_dwarfs,
				 neutron_stars, dt):
	"""
...
def advance_state(t, dt, num_stars, galactic_area, gas_mass,
				  immortal_stars, mortal_stars, black_holes,
				  white_dwarfs, neutron_stars):
	"""
	...
	infalling_gas_density = (dt * gas_infall_rate(t) *
							 gas_infall_abundances(elements))
	...
	star_forming_gas_density = (dt * star_formation_rate(sigma_gas) *
								gas_mass / sum(gas_mass))
	...
	mass_scaling_factor = (sum(star_forming_mass) /
						   total_stellar_mass(stars))
	...
	evolve_stars(mortal_stars, black_holes, white_dwarfs,
				 neutron_stars, dt)
...
def main():
	...
	while t < t_max:
		advance_state(t, dt, num_stars, galactic_area, gas_mass,
					  immortal_stars, mortal_stars, black_holes,
					  white_dwarfs, neutron_stars)
